...
#    def __init__(self, vregUnderTest, load):
    ...
    def calculateCurrent(self, data, hiLow):
        if self.vregUnderTest == "GFX":
            if hiLow:
                return data * 1000.0 / (5.0 * 8.7 / 16.0)
            else:
#                return data * 1000.0 / (40.0 * 8.7 / 16.0)
...
#            readBackLoad = float(readBackLoad7) + float(readBackLoad5) + float(readBackLoad9)
        elif self.vregUnderTest == "CPU":
...
#            chroma5Load = load / 3.0
#            self.chroma.channelArray[2].set_load(chroma5Load)
#            chroma7Load = load / 3.0
#            self.chroma.channelArray[3].set_load(chroma7Load)
#            chroma9Load = load / 3.0
#            self.chroma.channelArray[4].set_load(chroma9Load)
        elif self.vregUnderTest == "CPU":
